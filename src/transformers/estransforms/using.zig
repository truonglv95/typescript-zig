    ...
    fn visitSourceFile(self: *UsingDeclarationTransformer, visitor: *visitor_mod.NodeVisitor, node: ast_gen.NodeIndex) ast_gen.NodeIndex {
        ...
        if (usingKind != .None) {
            ...
            const helperText =
                ...
                \\});
            ;
            const helperIdent = self.transformer.factory.newIdentifier(helperText);
            topLevelStatements.insert(self.allocator, prologue.len, helperIdent) catch unreachable;
